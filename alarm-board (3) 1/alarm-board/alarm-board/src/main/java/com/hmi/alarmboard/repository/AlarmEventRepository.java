...
public interface AlarmEventRepository extends JpaRepository<AlarmEvent, Long> {
	...
	// Derived query methods with pagination
	Page<AlarmEvent> findByAlarmSeverityAndState(Severity severity, AlarmState state, Pageable pageable);

	Page<AlarmEvent> findByAlarmSeverity(Severity severity, Pageable pageable);
